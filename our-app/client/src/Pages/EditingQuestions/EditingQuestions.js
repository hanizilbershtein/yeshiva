...
const EditingQuestions = ({ categories }) => {
    ...
    try {
      async function fatch() {
        const { data } = await axios.post(
          "http://localhost:5000/questions/save",
          {
            saveSelect,
          }
        );
  ...
  return (
    <div>
      ...
      <input type={"text"} placeholder="הקלד שאלה" />
      <p>סווג את השאלה</p>
      {categories.map((el) => (
        <Question
          label={el.label}
      ...
      <button
        onClick={(e) => {
          setSaveSelect([
            {
              שאלה: e.target.parentNode.children[1].value,
              הגדרה:
                e.target.parentNode.children[3].children[2].children[2]
                  .children[0].children[0].innerText,
              מסכת: e.target.parentNode.children[4].children[2].children[2]
                .children[0].children[0].innerText,
              סוג: e.target.parentNode.children[5].children[2].children[2]
                .children[0].children[0].innerText,
              פרק: e.target.parentNode.children[6].children[2].children[2]
                .children[0].children[0].innerText,
              דף: e.target.parentNode.children[7].children[2].children[2]
                .children[0].children[0].innerText,
